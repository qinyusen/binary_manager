...
class Config:
    ...
    def get_default_db_path() -> str:
        db_path = os.environ.get(
            "RELEASE_PORTAL_DB", str(Config.BASE_DIR / "data" / "portal.db")
        )
    ...
    def get_storage_path() -> str:
        storage = os.environ.get(
            "RELEASE_PORTAL_STORAGE", str(Config.BASE_DIR / "storage")
        )
    ...
    def get_secret_key() -> str:
        secret = os.environ.get("RELEASE_PORTAL_SECRET")
        if not secret:
            raise RuntimeError(
                "RELEASE_PORTAL_SECRET environment variable is required. "
                "Set it before starting the application:\n"
                "  export RELEASE_PORTAL_SECRET='your-secure-secret-key'"
            )
        return secret
