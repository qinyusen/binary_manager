...
class ReleaseService:
    ...
    def __init__(
        ...
        self._test_validator = (
            PrePublishValidator() if enable_pre_publish_tests else None
        )
    ...
    def create_draft(
        ...
        self._log(
            AuditAction.RELEASE_CREATE, publisher_id, release, {"version": version}
        )
        return release
    ...
    def add_package(
        ...
        self._repo.save(release)
        self._log(
            AuditAction.UPLOAD_PACKAGE,
            user_id,
            release,
            {"content_type": str(content_type.value), "package_id": package_id},
        )
        return package_id
    ...
    def archive_release(
        self, release_id: str, user_id: Optional[str] = None
    ) -> Release:
        """归档版本"""
        release = self._get_and_validate_release(release_id, user_id)
        release.archive()
        self._repo.save(release)
        self._log(
            AuditAction.RELEASE_ARCHIVE, user_id, release, {"version": release.version}
        )
        return release
    ...
    def list_releases(
        self, resource_type: Optional[ResourceType] = None, status: Optional[str] = None
    ):
        """列出发布"""
        if resource_type:
    ...
    def _get_and_validate_release(
        ...
        if (
            user_id
            and self._auth
            and not self._auth.can_publish(user_id, release.resource_type)
        ):
            raise ValueError(f"无权发布 {release.resource_type.value} 类型的资源")

        return release

    def _run_tests_if_needed(
        self, release_id: str, run_tests: Optional[bool], test_level: Optional[str]
    ) -> None:
        """按需运行测试"""
