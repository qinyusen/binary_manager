...
def list_backups():
    ...
    try:
        ...
        backup_service = BackupService(
            db_path=db_path, storage_path=storage_path, backup_dir=backup_dir
        )
...
def create_backup():
    ...
    try:
        ...
        backup_info = backup_service.create_backup(
            name=name, include_storage=include_storage
        )
...
def get_backup_info(backup_filename: str):
    ...
    try:
        ...
        if not backup_info:
            return jsonify(
                {"error": "Not Found", "message": f"Backup {backup_filename} not found"}
            ), 404
...
def restore_backup():
    ...
    try:
        ...
        if not data or "backup_filename" not in data:
            return jsonify(
                {"error": "Bad Request", "message": "backup_filename is required"}
            ), 400
