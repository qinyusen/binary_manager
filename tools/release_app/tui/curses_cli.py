...
class CursesCLI:
    ...
    def _main_screen(self):
        """主菜单界面"""
        menu = Menu(["发布新版本", "查看历史版本", "退出"], "Release App - 主菜单")
        ...
        form = Form(
            {
                ...
                "notes": {"label": "发布说明", "default": ""},
                "binary_name": {"label": "二进制文件名 (可选)", "default": ""},
            },
            "版本信息",
        )
    ...
    def _confirm_release(self, release_type: str, version_info: Dict) -> bool:
        ...
        type_names = {"binary": "仅二进制", "commit": "仅提交", "both": "完整发布"}
    ...
    def _do_release(self, release_type: str, version_info: Dict):
        ...
        try:
            ...
            if release_type == "binary":
                ...
                result = self.manager.release_commit(version, notes)
            else:  # both
    ...
    def _show_version_detail(self, version_data: Dict):
        ...
        type_name = {"binary": "二进制", "commit": "提交", "both": "完整"}.get(
            version_data.get("release_type", ""), "未知"
        )
    ...
    def _show_git_info(self, y: int, x: int):
        ...
        try:
            sys.path.insert(
                0,
                str(Path(__file__).parent.parent.parent / "binary_manager_v2" / "core"),
            )
            ...
            safe_addstr(
                self.stdscr, y + 1, x, f"  分支: {git_info.get('branch', 'N/A')}"
            )
            safe_addstr(
                self.stdscr, y + 2, x, f"  提交: {git_info.get('commit_short', 'N/A')}"
            )
            safe_addstr(
                self.stdscr, y + 3, x, f"  作者: {git_info.get('author', 'N/A')}"
            )
            safe_addstr(
                self.stdscr,
                y + 4,
                x,
                f"  状态: {'有未提交更改' if git_info.get('is_dirty') else '干净'}",
            )
        except Exception as e:
            logger.debug(f"Git信息加载失败: {str(e)}")
