...
class TestReleaseService:
    ...
    def test_publish_release_without_binary_fails(self, service, mock_repository):
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="发布必须包含至少一个二进制包"):
    ...
    def test_add_package_checks_permission(
        self, service, mock_repository, mock_auth_service
    ):
        """测试添加包时检查权限"""
        # Arrange
        ...
        # Act & Assert
        with pytest.raises(ValueError, match="无权发布"):
            service.add_package(
                release_id="rel_123",
                content_type=ContentType.BINARY,
                source_dir="/path/to/source",
...
class TestReleaseServiceWithTests:
    ...
    def test_publish_with_tests_pass(self, service_with_tests, mock_repository):
        ...
        test_result = TestResult(
            passed=True, total_tests=10, passed_tests=10, failed_tests=0, duration=30.0
        )
        with patch.object(
            service_with_tests._test_validator,
            "validate_before_publish",
            return_value=test_result,
        ):
            # Act
            result = service_with_tests.publish_release(
                release_id="rel_123", run_tests=True
            )
    ...
    def test_publish_with_tests_fails(self, service_with_tests, mock_repository):
        ...
        test_result = TestResult(
            passed=False, total_tests=10, passed_tests=5, failed_tests=5, duration=30.0
        )
        with patch.object(
            service_with_tests._test_validator,
            "validate_before_publish",
            ...
            with pytest.raises(ValueError, match="测试失败"):
                service_with_tests.publish_release(release_id="rel_123", run_tests=True)
...
class TestReleaseServiceEdgeCases:
    ...
    def service(self, mock_repository, mock_storage):
        return ReleaseService(
            release_repository=mock_repository, storage_service=mock_storage
        )
