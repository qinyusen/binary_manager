...
from release_portal.domain.entities.license import License
...
class TestDownloadService:
    ...
    def test_get_available_packages_full_access(
        ...
        mock_storage.get_package_info = Mock(
            side_effect=[
    ...
    def test_get_available_packages_binary_access(
        self, service, mock_release_repo, mock_storage, mock_auth
    ):
        """测试受限访问权限只能下载二进制和文档"""
        # Arrange
        ...
        mock_release_repo.find_by_id = Mock(return_value=release)
        mock_auth.can_download_content = Mock(
            side_effect=[
                False,  # SOURCE
                True,  # BINARY
                True,  # DOCUMENT
    ...
    def test_list_downloadable_releases(
        self, service, mock_user_repo, mock_release_repo, mock_auth
    ):
        """测试列出可下载的发布"""
        # Arrange
        user = Mock()
